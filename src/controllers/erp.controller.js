...
class ErpController {
   ...
   */
  async logAudit(req, action, recordId, oldValue = null, newValue = null, transaction = null) {
    try {
  ...
  createCustomerWithContract = async (req, res, next) => {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= attempts; attempt += 1) {
        if (shouldGenerateStringId) payload.id = await generateScopedSequentialId(this.model, req.companyId);
        try {
      ...
      await this.logAudit(req, "CREATE", newItem.id, null, newItem.toJSON(), transaction);
  ...
  updateCustomerWithContract = async (req, res, next) => {
    ...
    try {
      ...
      transaction = await models.sequelize.transaction();
      const item = await this.model.findOne({ where, transaction, lock: transaction.LOCK.UPDATE });
      if (!item) throw new NotFoundError(`${this.model.name} record not found.`);

      if (req.body?.expectedUpdatedAt !== undefined) {
        const expectedDate = new Date(req.body.expectedUpdatedAt);
        if (Number.isNaN(expectedDate.getTime())) {
          throw new AppError("expectedUpdatedAt is invalid.", 422, "INVALID_EXPECTED_UPDATED_AT", { expectedUpdatedAt: ["expectedUpdatedAt must be a valid timestamp."] });
        }
        if (expectedDate.getTime() !== new Date(item.updatedAt).getTime()) {
          throw new AppError("Customer was changed by another request.", 409, "CUSTOMER_UPDATE_CONFLICT", { expectedUpdatedAt: ["Customer has changed since it was loaded."] });
        }
      }
  ...
  create = async (req, res, next) => {
    try {
      if (this.model.name === "Customer") return this.createCustomerWithContract(req, res, next);
  ...
  update = async (req, res, next) => {
    try {
      if (this.model.name === "Customer") return this.updateCustomerWithContract(req, res, next);
      const where = {
          id: req.params.id,
          ...(this.model.rawAttributes.companyId ? { companyId: req.companyId } : {})
      };
