...
router.get("/policies/history", authMiddleware, requirePermission(policyService.READ_PERMISSION), async (req, res, next) => {
  try {
    ...
    const data = await policyService.listPolicyHistory({ companyId: req.companyId, businessContext: req.query.businessContext || "CGP", karat, scopeType: req.query.scopeType || null });
