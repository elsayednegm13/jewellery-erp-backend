...
const profile = require("../services/loose-pearl-profile.service");
...
async function context(req) {
  ...
  return { settings, taxPolicy, masters, pearlSizes, locations, suppliers: suppliers.filter((row) => String(row.status || "").toLowerCase() !== "inactive"), barcode };
}

router.get("/contract", ...readGuard, async (req, res, next) => {
  try { return res.status(200).json({ success: true, readOnly: true, data: profile.contract(await context(req)) }); }
...
router.post("/preview", ...readGuard, async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, readOnly: true, data: profile.calculatePreview({ input, taxPolicy: data.taxPolicy, masters: data.masters, pearlSizes: data.pearlSizes }) });
  } catch (error) {
    if (!error.statusCode) { const wrapped = new ValidationError(error.message || "Loose Pearl preview failed."); wrapped.errorCode = error.errorCode || error.code || error.message; return next(wrapped); }
    return next(error);
  }
});
