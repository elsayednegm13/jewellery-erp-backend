...
const cgpBusinessViewService = require("../services/cgp-business-view.service");
const financialSettlementService = require("../services/financial-settlement.service");
...
function bind(kind) {
  ...
  if (kind === "cgp") {
    router.get("/cgp/drafts/:id/business-view", authMiddleware, async (req, res, next) => {
      try {
        const names = new Set(await permissionService.getUserPermissionNames(req.user));
        if (!names.has("gold_purchase.cgp.view_integration_status") && !names.has("gold_purchase.cgp.view") && !(req.user?.accountType === "super_admin") && !["admin", "owner"].includes(req.user?.role)) {
          throw new ForbiddenError("gold_purchase.cgp.view_integration_status is required");
        }
        const data = await cgpBusinessViewService.getBusinessView({ context: context(req), id: req.params.id });
        return res.status(200).json({ success: true, data });
      } catch (error) { return next(error); }
    });
    ...
    // Thin transport for the Accounting/Treasury-owned settlement authority.
    // This route never creates its own journal, Treasury row, or liability math.
    router.post("/cgp/drafts/:id/settlements", authMiddleware, async (req, res, next) => {
      try {
        const names = new Set(await permissionService.getUserPermissionNames(req.user));
        if (!names.has("gold_purchase.cgp.settle")) throw new ForbiddenError("gold_purchase.cgp.settle is required");
        const document = await draftService.findScoped("cgp", context(req), req.params.id, null, { includeVoided: true });
        if (String(document.businessStatus).toUpperCase() !== "POSTED") {
          throw new AppError("Only a posted CGP can receive a settlement.", 409, "CGP_SETTLEMENT_STATE_INVALID");
        }
        if (!document.postingReference) throw new AppError("Posted CGP accounting reference is missing.", 409, "CGP_SETTLEMENT_SOURCE_INVALID");
        const body = req.body || {};
        const liability = await models.CustomerFinancialLiability.findOne({ where: {
          ...
          sourceType: "CUSTOMER_GOLD_PURCHASE_POSTED",
        } });
        if (!liability) throw new AppError("Settlement liability does not belong to this posted CGP.", 409, "CGP_SETTLEMENT_LIABILITY_SOURCE_INVALID");
        const paymentMethod = String(body.paymentMethod || "").trim().toUpperCase();
        const cashAmount = body.cashAmount == null || body.cashAmount === "" ? null : body.cashAmount;
        const bankAmount = body.bankAmount == null || body.bankAmount === "" ? null : body.bankAmount;
        const legs = [];
        if (["CASH", "MIXED"].includes(paymentMethod) && cashAmount != null) legs.push({ method: "CASH", amount: cashAmount });
        if (["BANK", "BANK_TRANSFER", "MIXED"].includes(paymentMethod) && bankAmount != null) legs.push({ method: "BANK_TRANSFER", amount: bankAmount, bankReference: body.bankReference });
        if (!legs.length && paymentMethod === "CASH" && cashAmount != null) legs.push({ method: "CASH", amount: cashAmount });
        if (!legs.length && paymentMethod === "BANK" && bankAmount != null) legs.push({ method: "BANK_TRANSFER", amount: bankAmount, bankReference: body.bankReference });
        if (!legs.length) throw new AppError("Settlement payment method and amount are required.", 422, "FINANCIAL_SETTLEMENT_REQUEST_INVALID");
        const data = await financialSettlementService.executeCustomerPayoutSettlement({
          context: { companyId: req.companyId, branchId: req.branchId, actorId: req.user.id },
          input: {
