...
const looseDiamond = require("../services/loose-diamond-profile.service");
...
async function context(req) {
  ...
  const categories = profileMasterDataService.categoriesForProfile(looseDiamond.PROFILE);
  ...
  const locations = branchId ? await models.sequelize.query(
    `SELECT id,code,name,location_type AS "locationType",is_active AS "isActive"
      ...
      ORDER BY name,id`,
    { replacements: { companyId: req.companyId, branchId }, type: models.sequelize.QueryTypes.SELECT }
  ) : [];
  const suppliers = await models.Supplier.findAll({
    where: { companyId: req.companyId },
    attributes: ["id", "name", "status", "taxNumber"],
    order: [["name", "ASC"]],
    raw: true,
  });
...
router.get("/contract", ...readGuard, async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, readOnly: true, data: looseDiamond.contract(data) });
  } catch (error) {
    return next(error);
  }
});
...
router.post("/preview", ...readGuard, async (req, res, next) => {
  try {
    ...
    const result = looseDiamond.calculatePreview({ input, taxPolicy: data.taxPolicy, masters: data.masters });
    return res.status(200).json({ success: true, readOnly: true, data: result });
  } catch (error) {
    if (!error.statusCode) {
      const wrapped = new ValidationError(error.message || "Loose Diamond preview failed.");
      wrapped.errorCode = error.errorCode || error.code || error.message;
      return next(wrapped);
    }
    return next(error);
  }
