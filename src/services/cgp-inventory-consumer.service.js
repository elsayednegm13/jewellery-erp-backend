...
async function createAssetProjection({ transaction, event, document, item, snapshot, line, failureInjector, itemIndex }) {
  ...
  const now = new Date();
  const description = String(line.itemDescription || item.notes || "").trim();
  const fallbackName = `ذهب مستعمل ${Number(snapshot.karat)}K`;
  const assetName = description || fallbackName;
  const asset = await models.Asset.create({
    id: inventoryRuntime.newId("CGPA"),
    companyId: document.companyId,
    name: assetName,
    description: description || `Customer Gold Purchase ${document.draftNumber}, line ${item.lineNumber}`,
    type: "gold-weight",
