...
class GoldPriceService {
  ...
  /**
   * Prices a jewellery item: metal value + (physical grams × making/g) + stone value, + VAT.
   *   metalValue = grossWeight × perGram(karat)
   */
  async quoteItem({ grossWeight = 0, karat = 21, makingChargePerGram = 0, stoneValue = 0, currency = "AED", vatRate = 0, perGram = null }) {
    ...
    const totalMakingCharge = Math.round((Number(grossWeight) * Number(makingChargePerGram)) * 100) / 100;
    const subtotal = Math.round((metalValue + totalMakingCharge + Number(stoneValue)) * 100) / 100;
    ...
    return {
      ...
      totalMakingCharge,
      // Compatibility: historical consumers read `makingCharge` as the
      // monetary component of the quote; new input semantics are explicit in
      // makingChargePerGram and the returned total is authoritative.
   ...
   *  - dynamic_by_karat_plus_making→ goldWeight × perGram + (goldWeight × makingChargePerGram) + stoneValue.
   *
  ...
  computeItemSalePrice({
    ...
    switch (mode) {
      ...
      case "dynamic_by_karat_plus_making":
        return round(metal + (Number(goldWeight) * Number(makingChargePerGram)) + Number(stoneValue));
