...
async function calculateExpected(session, options = {}) {
  const expected = await calculateExpectedDecimal(session, options);
  return Number(expected.toFixed(4));
}

async function calculateExpectedDecimal(session, options = {}) {
  ...
  return new Decimal(String(session.openingCountedAmount || 0)).plus(new Decimal(String(movement || 0)));
}
