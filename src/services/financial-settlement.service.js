...
const policy = require("./financial-approval-policy.service");
...
async function executeCustomerPayoutSettlement({ context, input, modelSet = models }) {
  ...
  try {
    return await modelSet.sequelize.transaction(async (transaction) => {
      ...
      const evaluation = await policy.evaluateFinancialApprovalPolicy({ models: modelSet, operationType: OPERATION_TYPE, companyId: context.companyId, branchId: context.branchId, currency: liability.currency, paymentMethod, amount: total.toFixed(4), transaction });
      if ([policy.DECISION.POLICY_CONFIGURATION_MISSING, policy.DECISION.POLICY_CONFIGURATION_AMBIGUOUS, policy.DECISION.INVALID_CONTEXT].includes(evaluation.status)) fail("Financial approval policy cannot authorize this settlement.", evaluation.status);
      let approvalRequest = null;
      if (evaluation.status === policy.DECISION.APPROVAL_REQUIRED) {
        if (!input.approvalRequestId) fail("Human financial approval is required before settlement execution.", "FINANCIAL_APPROVAL_REQUIRED", 409);
        approvalRequest = await modelSet.ApprovalRequest.findOne({ where: { id: String(input.approvalRequestId), companyId: context.companyId, type: policy.FINANCIAL_OPERATION_TYPE, status: "approved", policyId: evaluation.policy.id, subjectType: "CustomerFinancialLiability", subjectId: liability.id, branchId: context.branchId, currency: liability.currency, paymentMethod }, transaction, lock: transaction.LOCK.KEY_SHARE });
        if (!approvalRequest || !new Decimal(approvalRequest.amount).eq(total)) fail("Approved financial approval evidence does not match this settlement.", "FINANCIAL_APPROVAL_REQUIRED", 409);
      }
      ...
      const settlement = await modelSet.FinancialSettlement.create({
        ...
        currency: liability.currency, totalAmount: total.toFixed(4), status: "EXECUTED", approvalPolicyId: evaluation.policy.id,
        approvalPolicyVersion: evaluation.policy.version, approvalDecisionSnapshot: evaluation.decisionSnapshot, approvalRequestId: approvalRequest?.id || null,
