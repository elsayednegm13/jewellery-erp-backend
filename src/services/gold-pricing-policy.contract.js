...
function assertQuoteUsable(quote, { currency, staleAfterSeconds = 120, now = new Date(), quoteType } = {}) {
  if (!quote || quote.status && quote.status !== QUOTE_STATUSES.VALID) throw new AppError("A valid market quote is required", 422, "GOLD_MARKET_QUOTE_UNAVAILABLE");
  if (String(quote.currency || "").toUpperCase() !== String(currency || "").toUpperCase()) throw new AppError("Market quote currency does not match the transaction", 422, "GOLD_PRICING_CURRENCY_MISMATCH");
  if (!isQuoteFresh(quote, staleAfterSeconds, now)) throw new AppError("Market quote is stale", 422, "GOLD_PRICING_QUOTE_STALE");
  try { return assertQuoteTypeAvailable(quote, quoteType); } catch { throw new AppError("Required market quote type is unavailable", 422, "GOLD_PRICING_QUOTE_TYPE_REQUIRED"); }
}
...
function calculateEffectiveRate({ quote, policy, companyId, karat, currency, now = new Date(), staleAfterSeconds = 120, marketQuoteId = null } = {}) {
  ...
  const selectedBaseRate = directRate || normalizedDecimal(({ BID: quote.bid, SPOT: quote.spot, ASK: quote.ask })[baseQuoteType], "baseMarketRate", { allowZero: false });
  const adjustmentValue = normalizedDecimal(policy.adjustmentValue, "adjustmentValue");
  let effective = selectedBaseRate;
  if (policy.adjustmentType === ADJUSTMENT_TYPES.FIXED_PER_GRAM) effective = selectedBaseRate.plus(adjustmentValue);
  else if (policy.adjustmentType === ADJUSTMENT_TYPES.PERCENTAGE) effective = selectedBaseRate.mul(new Decimal(1).plus(adjustmentValue.div(100)));
  ...
  return Object.freeze({
    ...
    baseQuoteType,
    baseMarketRate: selectedBaseRate.toDecimalPlaces(MARKET_RATE_SCALE, ROUNDING_MODE).toFixed(MARKET_RATE_SCALE),
    baseRateSource: directRate ? "PROVIDER_DIRECT_KARAT" : "QUOTE_TYPE",
    ...
    precision: { marketRateScale: MARKET_RATE_SCALE, adjustmentScale: MARKET_RATE_SCALE, finalRateScale: FINAL_RATE_SCALE, roundingMode: "HALF_UP", purityApplication: "NONE_IN_PRICING_ENGINE_KARAT_RATE" },
  });
}
