...
function calculateReceiptGoldValuation({ profile, weights, input, configuredVatRate }) {
  ...
  if (profile === BAR_PROFILE) {
    ...
    // A zero certificate amount has no taxable base, so a VAT rate is not
    // required for the no-certificate Gold Bar path.  Preserve fail-closed
    // behavior whenever a positive certificate amount actually needs VAT.
    const purchaseVat = resolveVatRate({ requestedRate: value.vatRate, configuredRate: configuredVatRate, required: certificateCost.gt(0) });
    const currentVat = resolveVatRate({ requestedRate: value.currentVatRate ?? value.vatRate, configuredRate: configuredVatRate, required: currentCertificateCost.gt(0) });
    ...
    return Object.freeze({
      purchase: Object.freeze({
        purchaseGoldRate: fixed(purchaseGoldRate), goldRateSource: value.purchaseRateSource || "MANUAL", goldValue: purchaseGoldValue,
      ...
      current: Object.freeze({
        rateSource: value.currentRateSource || "MANUAL", goldRate: fixed(currentGoldRate), goldValue: currentGoldValue, makingValue: null,
  ...
  return Object.freeze({
    ...
    current: Object.freeze({
      rateSource: value.currentRateSource || "MANUAL", goldRate: fixed(currentGoldRate), goldValue: currentGoldValue, makingValue: currentMakingValue,
...
function calculateCurrentGoldValuation({ profile, goldDetails, input, configuredVatRate, canonicalCurrentGoldRate = null }) {
  ...
  const currentGoldRate = decimal(canonicalCurrentGoldRate ?? value.currentGoldRate, "CURRENT_GOLD_RATE", { required: true, min: 0 });
  const currentRateSource = canonicalCurrentGoldRate === null || canonicalCurrentGoldRate === undefined ? "MANUAL" : "GOLD_CENTER";
  const currentGoldValue = goldValue(currentGoldRate, netGoldWeight);
  if (profile === BAR_PROFILE) {
  ...
  return Object.freeze({ rateSource: currentRateSource, goldRate: fixed(currentGoldRate), goldValue: currentGoldValue, makingValue, certificateValue: "0.00000000", componentValue: null, vatRate: vat.rate, vatRateSource: vat.source, vatBase, vatAmount, totalValue: fixed(new Decimal(currentGoldValue).plus(makingValue).plus(vatAmount)) });
}
