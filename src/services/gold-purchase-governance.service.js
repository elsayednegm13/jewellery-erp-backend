...
/**
 * Approval status is persisted governance history.  Actionability is a
 * separate read-model fact derived from the linked aggregate state so a
 * posted/reversed CGP request remains visible without exposing an action that
 * the immutable command will reject.
 */
function deriveApprovalActionability({ aggregateType, approvalStatus, linkedDocument = null }) {
  const pending = String(approvalStatus || "").toLowerCase() === "pending";
  if (aggregateType === "cgp") {
    if (!linkedDocument) {
...
async function enrichApprovalRows(rows, context) {
  const cgpIds = [...new Set(rows.filter((row) => row.aggregateType === "cgp").map((row) => row.documentId))];
  const linkedDocuments = cgpIds.length
    ? await models.CustomerGoldPurchaseDocument.findAll({
      where: { companyId: context.companyId, id: { [Op.in]: cgpIds } },
      attributes: ["id", "businessStatus", "branchId"]
    })
    : [];
  const byId = new Map(linkedDocuments.map((document) => [document.id, document]));
  return rows.map((row) => ({
    ...row.toJSON(),
    ...deriveApprovalActionability({
      aggregateType: row.aggregateType,
      approvalStatus: row.approvalStatus,
      linkedDocument: row.aggregateType === "cgp" ? byId.get(row.documentId) : null
    })
  }));
}
...
async function listApprovals(context, query = {}) {
  ...
  const enriched = await enrichApprovalRows(visible, context);
  const start = (page - 1) * limit;
  return { items: enriched.slice(start, start + limit), pagination: { total: visible.length, page, limit, pages: Math.ceil(visible.length / limit) }, filters: { ...query } };
}
...
async function getApproval(context, id) {
  ...
  return (await enrichApprovalRows([row], context))[0];
}
...
module.exports = { submit, review, createRevision, listApprovals, getApproval, history, hashSnapshot, snapshotFor, assertAction, deriveApprovalActionability };
