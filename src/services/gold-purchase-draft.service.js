...
async function list(kind, context, query, includeVoided) {
  ...
  if (kind === "cgp" && items.length > 0) {
    // One batch query keeps the list badge read-only and avoids an N+1
    // liability lookup. Drafts without a posted Liability remain UNPAID.
    const liabilities = await models.CustomerFinancialLiability.findAll({
      where: { companyId: context.companyId, sourceDocumentId: { [Op.in]: items.map((item) => item.id) } },
      attributes: ["sourceDocumentId", "originalAmount", "settledAmount", "outstandingAmount"],
    });
    const byDocument = new Map(liabilities.map((liability) => [liability.sourceDocumentId, liability]));
    for (const item of items) {
      const liability = byDocument.get(item.id);
      item.paymentStatus = liability
        ? buildPaymentSummary({ originalAmount: liability.originalAmount, settledAmount: liability.settledAmount, outstandingAmount: liability.outstandingAmount }).paymentStatus
        : "UNPAID";
    }
  }
  return { items, pagination: { total: result.count, page, limit, pages: Math.ceil(result.count / limit) }, filters: { ...query } };
}
