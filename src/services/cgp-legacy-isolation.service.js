...
// canonical CGP cutover is fail-closed: absent/unknown configuration blocks
// new legacy acquisition.  Only an explicit server-side `false` is an
// emergency compatibility opt-out; it is never granted by the UI or request.
...
function isCanonicalCgpCutoverActive(env = process.env) {
  const raw = String(env[CGP_LEGACY_ISOLATION_ENV] ?? "").trim().toLowerCase();
