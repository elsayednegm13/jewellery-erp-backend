...
async function createAudit({ models, companyId, branchId, auditNumber, auditMethod, locationId = null, notes = null, actor = {}, transaction, recordAudit = null }) {
  ...
  if (existing) {
    if (existing.branchId !== branchId || existing.auditMethod !== method) throw new ConflictError("Audit number body conflict.");
...
async function startAudit({ models, companyId, branchId, auditId, transaction }) {
  ...
  const assets = await models.Asset.findAll({
    where: { companyId, branchId, operationalStatus: { [Op.notIn]: ["SOLD", "MELTED"] } },
    transaction,
  });
...
async function observeAudit({ models, companyId, branchId, auditId, assetIds = [], barcodes = [], rfidNumbers = [], method = null, transaction }) {
  ...
  const requested = [...new Set([...assetIds, ...barcodes, ...rfidNumbers].map(String))];
  ...
  if (!assets.length) throw new ValidationError("No scanned Inventory Asset was found.");
  const observed = [];
  for (const asset of assets) {
    ...
    if (expected) {
      ...
      await models.StockAuditItem.create({
        id: inventoryV2Runtime.newId("IMAUDITEM"), stockAuditId: audit.id, assetId: asset.id,
        expectedBranchId: asset.branchId, scannedBranchId: branchId, status: "unexpected",
        result: "EXTRA", observedAt: new Date(), scanMethod,
      }, { transaction });
    }
