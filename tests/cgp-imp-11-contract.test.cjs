...
assert.equal(isCanonicalCgpCutoverActive({}), true);
assert.equal(isCanonicalCgpCutoverActive({ [CGP_LEGACY_ISOLATION_ENV]: "false" }), false);
assert.equal(isCanonicalCgpCutoverActive({ [CGP_LEGACY_ISOLATION_ENV]: "true" }), true);
assert.throws(() => assertLegacyCustomerGoldAcquisitionAllowed({ env: {} }), (error) => error instanceof AppError && error.errorCode === "CGP_LEGACY_ACQUISITION_ISOLATED");
assert.doesNotThrow(() => assertLegacyCustomerGoldAcquisitionAllowed({ env: { [CGP_LEGACY_ISOLATION_ENV]: "false" } }));
...
assert.equal(goldPurchaseRoutes.includes("/cgp/drafts/:id/settlements"), true, "canonical CGP settlement route must remain available");
assert.equal(goldPurchaseRoutes.includes('gold_purchase.cgp.settle'), true, "settlement must use dedicated permission");
