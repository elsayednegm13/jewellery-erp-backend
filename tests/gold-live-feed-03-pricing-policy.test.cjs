...
test("direct provider karat rates are used once without a second purity multiplication", () => {
  ...
  assert.equal(result.baseRateSource, "PROVIDER_DIRECT_KARAT_SPOT");
  assert.equal(result.baseMarketRate, "301.12345678");
  assert.equal(result.karatMarketRate, "275.33333333");
...
test("BID and ASK karat rates are derived from the matching fine-gold quote type", () => {
  const bid = calculateEffectiveRate({ quote, policy: policy({ baseQuoteType: "BID" }), companyId: "COMPANY-1", karat: 21, currency: "AED", now });
  const ask = calculateEffectiveRate({ quote, policy: policy({ baseQuoteType: "ASK" }), companyId: "COMPANY-1", karat: 21, currency: "AED", now });
  assert.equal(bid.baseRateSource, "DERIVED_FROM_BASE_QUOTE");
  assert.equal(bid.baseMarketRate, "300.12345678");
  assert.equal(bid.karatMarketRate, "262.60802468");
  assert.equal(bid.effectiveRate, "262.6080");
  assert.equal(ask.baseMarketRate, "302.12345678");
  assert.equal(ask.karatMarketRate, "264.35802468");
});

test("quote type, freshness, currency, karat and effective-rate gates fail closed", () => {
  assert.throws(() => calculateEffectiveRate({ quote: { ...quote, bid: null }, policy: policy(), companyId: "COMPANY-1", karat: 24, currency: "AED", now }), (error) => error.errorCode === "GOLD_MARKET_QUOTE_TYPE_UNAVAILABLE");
  assert.throws(() => calculateEffectiveRate({ quote: { ...quote, quoteTimestamp: new Date("2026-08-10T11:50:00.000Z") }, policy: policy(), companyId: "COMPANY-1", karat: 24, currency: "AED", now }), (error) => error.errorCode === "GOLD_MARKET_QUOTE_STALE");
  assert.throws(() => calculateEffectiveRate({ quote, policy: policy(), companyId: "COMPANY-1", karat: 24, currency: "USD", now }), (error) => error.errorCode === "GOLD_MARKET_QUOTE_CURRENCY_MISMATCH");
  ...
  assert.throws(() => calculateEffectiveRate({ quote: { ...quote, bid: "0", karat24Rate: "0" }, policy: policy(), companyId: "COMPANY-1", karat: 24, currency: "AED", now }), (error) => ["GOLD_MARKET_QUOTE_TYPE_UNAVAILABLE", "GOLD_PRICING_EFFECTIVE_RATE_NON_POSITIVE"].includes(error.errorCode));
});
