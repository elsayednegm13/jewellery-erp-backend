/**
 * Sales/Invoices pagination Phase 1 — READ-ONLY contract test (GET only).
 *
 * Confirms /invoices honours page/pageSize and returns the metadata the sales
 * hook relies on, that pages don't overlap, and that search/filters narrow the
 * result. No writes (the backend list was already capable — no backend change).
 *
 * Run from repo root: node backend/scripts/verify-invoices-pagination.js
 */
...
(async () => {
  ...
  try {
    ...
    const p1 = await get("/invoices?page=1&pageSize=5&sortBy=createdAt&sortDirection=desc");
    check(p1.status === 200, "GET /invoices?page=1&pageSize=5 → 200");
    ...
    if (p1.json.total > 5) {
      const p2 = await get("/invoices?page=2&pageSize=5&sortBy=createdAt&sortDirection=desc");
      ...
      console.log("  (≤5 invoices — skipping multi-page overlap check)");
    }

    console.log("\nsearch + filters narrow the result:");
    const all = await get("/invoices?page=1&pageSize=250");
    const totalAll = all.json.total;
    const draftFilter = await get(`/invoices?page=1&pageSize=250&filters=${encodeURIComponent(JSON.stringify({ status: "paid" }))}`);
    check(draftFilter.status === 200 && draftFilter.json.total <= totalAll, "status filter narrows (or equals) the total");
    const search = await get("/invoices?page=1&pageSize=250&search=zzzzzznomatch");
