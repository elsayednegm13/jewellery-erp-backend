/**
 * Customers pagination Phase 2 — READ-ONLY contract test (GET only).
 *
 * Confirms /customers honours page/pageSize and returns the metadata the
 * customers hook/repository relies on, that pages don't overlap, and that
 * search/filters narrow the result. No writes (no backend change was made).
 *
 * Run from repo root: node backend/scripts/verify-customers-pagination.js
 */
...
(async () => {
  ...
  try {
    ...
    const p1 = await get("/customers?page=1&pageSize=5&sortBy=createdAt&sortDirection=desc");
    check(p1.status === 200, "GET /customers?page=1&pageSize=5 → 200");
    ...
    if (p1.json.total > 5) {
      const p2 = await get("/customers?page=2&pageSize=5&sortBy=createdAt&sortDirection=desc");
      ...
      console.log("  (≤5 customers — skipping multi-page overlap check)");
    }

    console.log("\nsearch + filters narrow the result:");
    const all = await get("/customers?page=1&pageSize=250");
    const totalAll = all.json.total;
    const statusFilter = await get(`/customers?page=1&pageSize=250&filters=${encodeURIComponent(JSON.stringify({ status: "active" }))}`);
    check(statusFilter.status === 200 && statusFilter.json.total <= totalAll, "status filter narrows (or equals) the total");
    const search = await get("/customers?page=1&pageSize=250&search=zzzzzznomatch");
