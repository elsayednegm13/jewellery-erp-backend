...
// Disposable rehearsal guard. It accepts only a task-created database with the
// exact prefix and runs only Migration 79; it rejects Persistent and Acceptance.
const path = require("path");
const { Sequelize } = require("sequelize");
const Umzug = require("umzug");
const { resolveDatabaseEnv } = require("../src/config/database-env");

const EXPECTED = "20260810020000-gold-cgp-pricing-policies.js";
const PREFIX = "darfus_erp_gold_live_feed_03_rehearsal_";
...
function resolveTarget(env = process.env) {
  ...
  if (!target || !target.startsWith(PREFIX) || target === PERSISTENT || target === ACCEPTANCE) fail("GOLD_LIVE_FEED_03_REHEARSAL_TARGET_REJECTED");
  const config = resolveDatabaseEnv({ ...env, DATABASE_URL: "", DB_NAME: target });
  if (config.database !== target) fail("GOLD_LIVE_FEED_03_REHEARSAL_TARGET_MISMATCH");
  return config;
}

async function main() {
  if (!process.argv.includes("--execute")) { console.log(JSON.stringify({ dryRun: true, expected: EXPECTED, prefix: PREFIX })); return; }
  ...
  try {
    ...
    if (current.db !== config.database) fail("GOLD_LIVE_FEED_03_REHEARSAL_ACTUAL_TARGET_MISMATCH");
    ...
    if (pending.length !== 1 || pending[0] !== EXPECTED) fail("GOLD_LIVE_FEED_03_REHEARSAL_PENDING_SET_REJECTED");
    ...
    if (Number(after.count) !== 1) fail("GOLD_LIVE_FEED_03_REHEARSAL_MIGRATION_NOT_RECORDED");
    console.log(JSON.stringify({ database: current.db, migration: EXPECTED, applied: 1 }));
  } finally { await sequelize.close(); }
}

main().catch((error) => { console.error(error.code || "GOLD_LIVE_FEED_03_REHEARSAL_FAILED"); process.exitCode = 1; });
